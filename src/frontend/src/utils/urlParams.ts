...
/**
 * Stores the podcast guest type selection for a specific visit ID
 * Used to prefill the guest type when navigating from reception to podcast details
 *
 * @param visitId - The visit ID to associate with the guest type
 * @param guestType - The guest type ('guest' or 'cohost')
 */
export function storePodcastGuestTypePrefill(visitId: string, guestType: string): void {
    const key = `podcast_guest_type_${visitId}`;
    storeSessionParameter(key, guestType);
}

/**
 * Retrieves and clears the podcast guest type prefill for a specific visit ID
 * Used in the podcast details page to prefill the guest type selection
 *
 * @param visitId - The visit ID to retrieve the guest type for
 * @returns The guest type if found, null otherwise
 */
export function getAndClearPodcastGuestTypePrefill(visitId: string): string | null {
    const key = `podcast_guest_type_${visitId}`;
    const guestType = getSessionParameter(key);
    if (guestType) {
        clearSessionParameter(key);
    }
    return guestType;
}
